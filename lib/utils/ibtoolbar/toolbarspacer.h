...
public:
	...
	/// add an "About" action at the right end of the toolbar
	/// if automatically finds the toolbar in the QMainWindow *mw
	/// This should be called after all other actions have been added
	static QAction *addAbout(QMainWindow *mw);
	/// old version
	static QAction *addAbout(QToolBar *tb, QWidget *parent, const char aboutSlot[]);
	/// add an "Quit" action to the toolbar
	/// if automatically finds the toolbar in the QMainWindow *mw
	/// This should be called before any other actions are added
	static QAction *addQuit(QMainWindow *mw);
private slots:
	void about();
};
