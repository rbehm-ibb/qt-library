...
{
	IpcComm *comm = new IpcComm(m_device);
	...
	if (comm->isValidPort())
	{
		...
		connect(comm, &IpcComm::lostPortError, this, &IpcCommThread::lostPortError);
		exec();
	}
	delete comm;
}
...
IpcComm::IpcComm(QString device, QObject *parent)
	: QObject(parent)
	, m_waitResp(false)
	, m_hasTxBuffer(false)
{
	...
	m_port = new IBSerialPort(device, 230400, this);
